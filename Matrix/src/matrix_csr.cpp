...
bool MatrixCSR::isSymmetrical() const noexcept
{
  if (_width != _height)
    return false;
...
size_t MatrixCSR::rotateJacobi(MatrixCSR& solution, double precision) noexcept
{
  ...
  size_t maxI, maxJ;
  double max, fi;
  MatrixCSR matricaPoworota(_width);
  MatrixCSR temp(_width);
  solution = identity(_width);
  solution.reserve(_width);
  matricaPoworota.reserve(_width * _width);
  temp.reserve(_width * _width);
  double fault = 0.0;
  double value;
  double sqr2 = sqrt(2);
...
size_t MatrixCSR::rotatedJacobi(MatrixCSR& coef, MatrixCSR& solution, double precision) const noexcept
{
  coef = *this;
